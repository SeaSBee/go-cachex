...
func (tm *TagManager) InvalidateByTag(ctx context.Context, keys []string) error {
	...
	for i := 0; i < len(keys); i += tm.config.BatchSize {
		...
		batch := keys[i:end]
		if err := tm.store.Del(ctx, batch...); err != nil {
...
func (tm *TagManager) persistTagMappings(ctx context.Context) error {
	...
	return tm.store.Set(ctx, key, data, tm.config.TagMappingTTL)
}
...
func (tm *TagManager) loadTagMappings() {
	...
	key := "cachex:tag_mappings"
	data, err := tm.store.Get(ctx, key)
	if err != nil {
		logx.Warn("Failed to load tag mappings", logx.ErrorField(err))
		return
	}
	...
	if err := json.Unmarshal(data, &tagMappings); err != nil {
