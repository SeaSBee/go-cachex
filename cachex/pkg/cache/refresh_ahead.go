...
type RefreshAheadConfig struct {
	...
	DefaultRefreshBefore time.Duration
...
func (dlm *DistributedLockManager) TryLock(ctx context.Context, key string, ttl time.Duration) (func() error, bool, error) {
	...
	err := dlm.store.Set(ctx, lockKey, []byte(lockValue), ttl)
	if err != nil {
	...
	unlock := func() error {
		return dlm.store.Del(ctx, lockKey)
	}
...
func (ras *RefreshAheadScheduler) executeRefresh(task *RefreshTask) {
	ctx, cancel := context.WithTimeout(ras.ctx, ras.config.LockTimeout)
	defer cancel()
	...
	if ras.config.EnableDistributedLock {
		unlock, lockAcquired, err = ras.lockManager.TryLock(ctx, task.Key, ras.config.LockTimeout)
		if err != nil {
	...
	// Get current TTL
	ttl, err := ras.store.TTL(ctx, task.Key)
	if err != nil {
	...
	if err := ras.store.Set(ctx, task.Key, valueBytes, ttl); err != nil {
